...
const Dropdown = ({ options, value, onChange }) => {
    ...
    return <div className="w-48 relative">
        ...
        onClick={handleClick}>
            {/* //a interrogação para saber se o valor retornado será undefined */}
            {value?.label || 'Select'}
             <GoChevronDown className="text-lg" />
        </div>
        {isOpen && <div className="absolute top-full border rounded p-3 shadow bg-white w-full">{ renderDropdown }</div>}
    </div>
}
